...
println("Prescribed circular motion (2D)")
println("  prescribed speed = ", case.V(tf))
println("  max |speed|      = ", maximum(abs.(vec(solver.state.speed_full))))
println("  max |T|          = ", maximum(abs.(solver.state.Tlω)))
println("  max |C|          = ", maximum(abs.(solver.state.Clω)))
